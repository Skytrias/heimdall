...
drawColorwheel :: proc(ctx: ^nvg.Context, x, y, w, h, t: f32) {
	...
	for i in 0..<6 {
		...
		paint = nvg.LinearGradient(ax,ay, bx,by, nvg.HSLA(a0/(math.PI*2),1.0,0.55,255), nvg.HSLA(a1/(math.PI*2),1.0,0.55,255))
...
drawSlider :: proc(ctx: ^nvg.Context, pos, x, y, w, h: f32) {
	...
	// Knob Shadow
	bg = nvg.RadialGradient(x+math.round(pos*w),cy+1, kr-3,kr+3, nvg.RGBA(0,0,0,64), nvg.RGBA(0,0,0,0))
	nvg.BeginPath(ctx)
...
loadDemoData :: proc(ctx: ^nvg.Context, data: ^DemoData) {
	...
	data.fontIcons = nvg.CreateFont(ctx, "icons", "entypo.ttf")
	if (data.fontIcons == -1) {
		panic("Could not add font icons.")
	}
	data.fontNormal = nvg.CreateFont(ctx, "sans", "Roboto-Regular.ttf")
	if (data.fontNormal == -1) {
		panic("Could not add font italic.")
	}
	data.fontBold = nvg.CreateFont(ctx, "sans-bold", "Roboto-Bold.ttf")
	if (data.fontBold == -1) {
		panic("Could not add font bold.")
	}
	data.fontEmoji = nvg.CreateFont(ctx, "emoji", "NotoEmoji-Regular.ttf")
	if (data.fontEmoji == -1) {
...
stopGPUTimer :: proc(timer: ^GPUtimer) {
	...
	for timer.ret <= timer.cur && available == 1 {
		...
		gl.GetQueryObjectiv(timer.queries[timer.ret % GPU_QUERY_COUNT], gl.QUERY_RESULT_AVAILABLE, &available)
	}
