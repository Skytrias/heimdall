...
loadFonts :: proc(ctx: ^nvg.Context) -> bool {
	font: int
	font = nvg.CreateFont(ctx, "sans", "Roboto-Regular.ttf")
	if font == -1 {
		fmt.printf("Could not add font regular.\n")
		return false
	}
	font = nvg.CreateFont(ctx, "sans-bold", "Roboto-Bold.ttf")
	if font == -1 {
...
main :: proc() {
	...
	fb := nvg_gl.CreateFramebuffer(ctx, int(100 * px_ratio), int(100 * px_ratio), { .REPEAT_X, .REPEAT_Y })
	...
	for !glfw.WindowShouldClose(window) {
...

		{
			...
			nvg.BeginPath(ctx)
			nvg.RoundedRect(ctx, 140 + math.sin(f32(t)*1.3)*100, 140 + math.cos(f32(t)*1.71244)*100, 250, 250, 20)
