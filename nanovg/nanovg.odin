...
__flattenPaths :: proc(ctx: ^Context) {
	...
	// fmt.eprintln("~~~~~~~~~~~~~", len(cache.paths))
	for j in 0..<len(cache.paths) {
		path := &cache.paths[j]
		pts := cache.points[path.first:]

		// // NOTE to stop crashing when accessing other higher points than 0		
		// if len(pts) == 1 {
		// 	continue
		// }
		...
		p1 := &pts[0]
		p0 := &pts[path.count-1]
		if __ptEquals(p0.x,p0.y, p1.x,p1.y, ctx.distTol) && path.count > 1 {
			path.count -= 1
			p0 = &pts[path.count - 1]
			path.closed = true
		}
...
// Returns handle to the font.
CreateFontMem :: proc(ctx: ^Context, name: string, slice: []byte, freeLoadedFont: bool) -> int {
	return fontstash.AddFontMem(&ctx.fs, name, slice, freeLoadedFont)
}
...
TextBreakLines :: proc(
	...
	for fontstash.TextIterNext(fs, &iter, &q) {
		...
		case '\t', '\v', '\f', ' ', 0x00a0: type = .Space // NBSP
		case '\n': type = pcodepoint == 13 ? .Space : .Newline
		case '\r': type = pcodepoint == 10 ? .Space : .Newline
		case 0x0085: type = .Newline // NEL
		case: 
